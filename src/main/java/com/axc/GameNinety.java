...
public class GameNinety {
    ...
    public static int[][] generateStripOfSixTickets(){
        ...
        for(int c=1; c<=9; c++){ // loop to columns
            ...
            if(c>=5) { checkIfFiveNumberRuleForceToPolulateSomeRows(c); }
...
//            }
    ...
    private static void fillTheListWithAvailableCellsOnAColumn() {
        ...
        c=1;  i = 0;  n = 9;  s=0;  e=9;  a=0;  //  row number 0...17, sR start Row  eR end Row
    ...
    public static void printTickets(){
        ...
        for (int row = 0; row < 18; row++) {
            ...
            for(int col = 0; col<9;col++) {
                val = RC[row][col];
                if(col == 0 && val !=0) { System.out.print(" "); }
                if( val !=0) { aR[row]--; }
                System.out.print("       " +
                        (val == 0 ? "--" : val ) + " "
                );
    ...
    private static void populateEachTicketWithOneNumberPerColumn(int c) {
        ...
        for(t=1; t<=6; t++){    // In a single tichet, on o column, put at least one number in one random row
            ...
            if(c < 5  ) {
                tr=R.nextInt(3);                 System.out.print(" tr="+tr); // choose a random row in one ticket
                Row = ((t-1)*3)+(tr);                     System.out.print(" Row="+Row);  // row in the matrix store of the row ticket
...
//                    if(debug) { RC[Row][c-1] = 93;  }                        // assign the randomized value in the strip tickets store
    ...
    private static void populateRemainingNumbersOnThisColumn(int c) {
        ...
        for(n=nM; n>0; --n, --mL, --nM){         // loop to random 9 cells to populate them with numbers
            ...
            if(c==9) { nM = Math.min(nM, mapR.get(c).size()); }
            ...
            if(nM > 0) {
                ...
                if(C[r] == 5) {  mapR.forEach((row, list) -> list.remove(removeRowValue)); }  // are 5 numbers populated on a row, so remove to not be used further
            }
